...
export async function add(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const question = await questionService.addQuestion(req.params.quizId as string, req.user!.id, req.body);
...
export async function reorder(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    await questionService.reorderQuestions(req.params.quizId as string, req.user!.id, req.body.questionIds);
